...
def _strip_json_comments(s: str) -> str:
    # Remove //... and /*...*/ comments (not valid JSON, but sometimes present)
    s = re.sub(r"(?m)//.*?$", "", s)
    s = re.sub(r"/\*.*?\*/", "", s, flags=re.S)
    return s

def _relaxed_json_loads(raw: str) -> Any:
    """Best-effort JSON parser for slightly-broken files.
    Fixes:
      - comments (//, /* */)
      - trailing commas before } or ]
      - missing commas between adjacent string/object/array values and the next key
    """
    s = raw.lstrip("\ufeff")
    ...
    # Example: "KGYX"  "notes": null  -> "KGYX", "notes": null
    # Also handles: } "foo": -> }, "foo":
    s = re.sub(r"(\"[^\"]*\"|\}|\]|null|true|false|[0-9.]+)\s*(\"[A-Za-z0-9_\- ]+\"\s*:)", r"\1, \2", s)

    return json.loads(s)

def _safe_read_json(path: Path, default: Any, *, relaxed: bool = False, return_error: bool = False):
    """Read JSON from disk safely.
    If relaxed=True, we attempt a small set of repairs to tolerate minor JSON damage.
    If return_error=True, returns (value, error_string_or_None).
    """
    try:
        if not path.exists():
            return (default, f"file not found: {path}") if return_error else default
        raw = path.read_text(encoding="utf-8", errors="replace")
        try:
            val = json.loads(raw)
            return (val, None) if return_error else val
        except Exception as e:
            if not relaxed:
                return (default, str(e)) if return_error else default
            try:
                val = _relaxed_json_loads(raw)
                return (val, f"repaired parse: {e}") if return_error else val
            except Exception as e2:
                return (default, f"{e}; repair_failed: {e2}") if return_error else default
    except Exception as e:
        return (default, str(e)) if return_error else default
...
_profiles, _perr = _safe_read_json(DZ_PROFILES_PATH, default={}, relaxed=True, return_error=True)
STORE = Store(
    profiles=_profiles,
    active=_safe_read_json(ACTIVE_DZ_PATH, default={"code": "", "active_profile_id": "a_license", "limits_overrides": {}}),
    dz_profiles_error=_perr,
)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        profiles, perr = _safe_read_json(DZ_PROFILES_PATH, default={}, relaxed=True, return_error=True)
        STORE.profiles = profiles
        STORE.dz_profiles_error = perr
